...
# - Enable training of a downstream model in target-speaker speech processing tasks.
#
# Original source: https://github.com/s3prl/s3prl/blob/v0.4.17/s3prl/downstream/runner.py
...
class Runner:
    ...
    def __init__(self, args, config):
        ...
        self.config = config
        self.init_ckpt = (
            torch.load(self.args.init_ckpt, map_location="cpu", weights_only=False) if self.args.init_ckpt else {}
        )

        self.upstream = self._get_upstream()
        # spk models
        self.spk_conditioned = self.config.get("speaker_conditioned", False)
        if self.spk_conditioned:
            # upstream
            self.featurizer_spk_k = self._get_featurizer("Featurizer_Spk_k")
            self.use_mhfa = self.config["downstream_expert"]["modelrc"]["spk_extractor"] == "MHFA"
            if self.use_mhfa:
                # use for MHFA (https://arxiv.org/abs/2210.01273)
                self.featurizer_spk_v = self._get_featurizer("Featurizer_Spk_v")

        self.featurizer = self._get_featurizer()
        self.downstream = self._get_downstream()
        self.all_entries = [self.upstream, self.featurizer, self.downstream]
        if self.spk_conditioned:
            self.all_entries.append(self.featurizer_spk_k)
            if self.use_mhfa:
                self.all_entries.append(self.featurizer_spk_v)
    ...
    def _get_upstream(self, name="Upstream"):
        if "from_hf_hub" in self.args and self.args.from_hf_hub == True:
            from huggingface_hub import snapshot_download

            print("[Runner] - Downloading upstream model self.args.upstream from the Hugging Face Hub")
    ...
    def _get_featurizer(self, name="Featurizer"):
        model = Featurizer(
    ...
    def _get_downstream(self):
        ...
        model = Downstream(
            ...
            upstream_dim_spk=self.featurizer.model.output_dim,
            **self.config,
            **vars(self.args),
        ).to(self.args.device)
        ...
        while pbar.n < pbar.total:
            ...
            for batch_id, (wavs, *others) in enumerate(
                ...
                try:
                    ...
                    if self.spk_conditioned:
                        spk_enrolls = others[-1]
                        spk_enrolls = [torch.FloatTensor(spk_enroll).to(self.args.device) for spk_enroll in spk_enrolls]

                    with torch.cuda.amp.autocast(enabled=amp):
                        if self.upstream.trainable:
                            features = self.upstream.model(wavs)
                            if self.spk_conditioned:
                                features_spk = self.upstream.model(spk_enrolls)
                        else:
                            with torch.no_grad():
                                features = self.upstream.model(wavs)
                                if self.spk_conditioned:
                        ...
                        if self.spk_conditioned:
                            features_spk_k = self.featurizer_spk_k.model(spk_enrolls, features_spk)
                            if self.use_mhfa:
                                features_spk_v = self.featurizer_spk_v.model(spk_enrolls, features_spk)
                            else:
                                features_spk_v = None
