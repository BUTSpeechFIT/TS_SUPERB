...
# - Enabled loading of enrollment speech.
#
...
class SeparationDataset(Dataset):
    def __init__(
        ...
        # MIX data
        self.base_dataset = LibriMix(self.csv_dir, "sep_noisy", self.rate, N_SRC, segment)
        data_aux = read_enrollment_csv(
            Path(self.csv_dir) / "mixture2enrollment.csv", self.base_dataset.df["mixture_ID"].tolist()
        )
    ...
    def __getitem__(self, i):
        ...
        tgt_spk_idx = int(utt_id.split("-")[0][1])
        ...
        source_path = row[f"source_{tgt_spk_idx}_path"]
