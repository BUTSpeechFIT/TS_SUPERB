...
#
# Original source: https://github.com/s3prl/s3prl/blob/v0.4.17/s3prl/downstream/asr/expert.py

import math  # use for kenlm decoding
import os
...
def get_decoder(decoder_args_dict, dictionary):
    ...
    if decoder_args.decoder_type == "kenlm":
        from ..asr.w2l_decoder import W2lKenLMDecoder
...
class DownstreamExpert(nn.Module):
    ...
    def __init__(self, upstream_dim, upstream_rate, downstream_expert, expdir, upstream_dim_spk, **kwargs):
        ...
        self.dictionary = Dictionary.load(
            self.datarc.get("dict_path", str(Path(__file__).parent.parent / "asr" / "char.dict"))
        )
        ...
        spk_conditioning_layer = model_conf.get("spk_conditioning_layer", 1)
        if model_conf["spk_conditioning_layer"] > 0:
            if isinstance(model_conf["dim"], list):
                out_spkemb_dim = model_conf["dim"][spk_conditioning_layer - 1]
            else:
                out_spkemb_dim = model_conf["dim"]
            bidirection = model_conf.get("bidirection", False)
            if bidirection:
                out_spkemb_dim *= 2
        else:
            out_spkemb_dim = self.modelrc["project_dim"]
        if self.modelrc["spk_extractor"] == "MHFA":
            self.spk_extractor = MHFA(head_nb=8, inputs_dim=self.upstream_dim_spk, outputs_dim=out_spkemb_dim)
        else:
    ...
    def _get_log_probs(self, features, spk_outemb):
        ...
        logits, log_probs_len = self.model(features, features_len, spk_outemb)
    ...
    # Interface
    def forward(self, split, features, features_k, features_v, labels, filenames, enroll_wav, records, **kwargs):
        """
        Args:
            split: string
                The name of the dataloader, can be train/dev/test-clean/test-other for asr
        ...
        if self.modelrc["spk_extractor"] == "MHFA":
            features_k = pad_sequence(features_k, batch_first=True).to(device=device)
            features_v = pad_sequence(features_v, batch_first=True).to(device=device)
            spk_outemb = self.spk_extractor(features_k, features_v).unsqueeze(1)

        log_probs, log_probs_len = self._get_log_probs(features, spk_outemb)
    ...
    def log_records(self, split, records, logger, global_step, batch_ids, total_batch_num, **kwargs):
        ...
        logger.add_scalar(f"ts_asr/{split}-loss", loss, global_step=global_step)
        logger.add_scalar(f"ts_asr/{split}-uer", uer, global_step=global_step)
        logger.add_scalar(f"ts_asr/{split}-wer", wer, global_step=global_step)
